...
export const Button = ({children, type, onClick, buttonType = 'button', overrides, testId = 'button-component'}) => {
  const buttonStyles = {
    'primary': 'bg-primaryColor py-1 px-4 text-textOnPrimaryBackground hover:bg-secondaryColor',
  ...
  const commonStyles = 'py-2 px-4 rounded block text-lg text-white hover:scale-105 transition duration-200';

  const styleClasses = `${commonStyles} ${buttonStyles[type]} ${overrides}`;

  return (
    <button type={buttonType} data-testid={testId} className={styleClasses} onClick={onClick} style={{minWidth: '150px'}}>
        {children}
    </button>
  );
};
